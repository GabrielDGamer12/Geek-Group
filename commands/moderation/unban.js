    ...
    run: async (client, message, args) => {
        ...
        message.guild.fetchBans().then( bans => {
            ...
            if (user) {
                ...
                message.guild.members.unban(user.user.id, reason).then(() => message.channel.send(embed)).then(m => m.delete({ timeout: 5000 }))
            } else {
            ...
            console.log(e)
            message.channel.send('Ocorreu um erro!').then(m => m.delete({ timeout: 5000 }))
        });
